...
class OccFusion(Base3DSegmentor):
    ...
    def extract_feat(self, img):
        ...
        print(f"    [EXTRACT] Reshape input: {(time.time() - reshape_start)*1000:.2f}ms")
        
        # Backbone forward
        backbone_start = time.time()
        img_feats = self.img_backbone(img)
        print(f"    [EXTRACT] Backbone forward: {(time.time() - backbone_start)*1000:.2f}ms")
        
        # Neck forward
        neck_start = time.time()
        img_feats = self.img_neck(img_feats)
        print(f"    [EXTRACT] Neck forward: {(time.time() - neck_start)*1000:.2f}ms")

        # Reshape output
        reshape_out_start = time.time()
        img_feats_reshaped = []
        for img_feat in img_feats:
            _, C, H, W = img_feat.size()
            img_feats_reshaped.append(img_feat.view(B, N, C, H, W))
        print(f"    [EXTRACT] Reshape output: {(time.time() - reshape_out_start)*1000:.2f}ms")
        print(f"    [EXTRACT] TOTAL extract_feat: {(time.time() - extract_start)*1000:.2f}ms")
    ...
    def gaussian(self, batch_inputs, batch_data_samples, voxel_feats):
        ...
        print(f"    [GAUSSIAN] Starting gaussian computation...")
        ...
        def gaussian_to_voxel_splatting_optimized(voxel_grid, gaussian_means, gaussian_covs, gaussian_logits, gaussian_geometry, voxel_size):
            ...
            print(f"        [GAUSS_SPLAT] V:{V}, G:{G}")
            
            init_start = time.time()
            voxel_logits = torch.zeros((V, gaussian_logits.shape[1] + gaussian_geometry.shape[1]), device=gaussian_logits.device)
            topk = 5
            print(f"        [GAUSS_SPLAT] Init: {(time.time() - init_start)*1000:.2f}ms")

            # Pure PyTorch implementation - much faster than FAISS for small datasets
            knn_start = time.time()
            ...
            if V * G > chunk_size * chunk_size:
                print(f"        [GAUSS_SPLAT] Using chunked computation (V*G={V*G} > {chunk_size*chunk_size})")
                indices_list = []
                ...
                print(f"        [GAUSS_SPLAT] Using standard computation")
            ...
            print(f"        [GAUSS_SPLAT] KNN computation: {(time.time() - knn_start)*1000:.2f}ms")
            ...
            print(f"        [GAUSS_SPLAT] Tensor operations: {(time.time() - tensor_ops_start)*1000:.2f}ms")

            # Time scatter operation
            scatter_start = time.time()
            voxel_logits = torch_scatter.scatter_add(weighted_logits, voxel_idx, dim=0, out=voxel_logits)
            print(f"        [GAUSS_SPLAT] Scatter operation: {(time.time() - scatter_start)*1000:.2f}ms")
            print(f"        [GAUSS_SPLAT] TOTAL splatting: {(time.time() - start_time)*1000:.2f}ms")
            
            return voxel_logits

        def hybrid_voxel_splat_loss(voxel_features, voxel_grid, voxel_labels, geometric_label):
            splat_loss_start = time.time()
            ...
            print(f"        [SPLAT_LOSS] Sampling: {(time.time() - sampling_start)*1000:.2f}ms")
            ...
            print(f"        [SPLAT_LOSS] Gaussian decoding: {(time.time() - decoder_start)*1000:.2f}ms")

            # Voxel splatting
            splatting_start = time.time()
            ...
            print(f"        [SPLAT_LOSS] Splatting call: {(time.time() - splatting_start)*1000:.2f}ms")
            ...
            print(f"        [SPLAT_LOSS] Loss computation: {(time.time() - loss_comp_start)*1000:.2f}ms")
            print(f"        [SPLAT_LOSS] TOTAL hybrid_voxel_splat_loss: {(time.time() - splat_loss_start)*1000:.2f}ms")
        ...
        print(f"    [GAUSSIAN] Data preparation: {(time.time() - prep_start)*1000:.2f}ms")
        ...
        for b in range(B):
            ...
            total_loss += loss
            print(f"        [BATCH {b}] Total batch time: {(time.time() - batch_iter_start)*1000:.2f}ms")

        print(f"    [GAUSSIAN] Batch processing: {(time.time() - batch_start)*1000:.2f}ms")
    ...
    def _forward(self, batch_inputs, batch_data_samples): # img, img_metas, sparse_voxel, voxel_coords, batch_data_samples
        ...
        print(f"[DEBUG] 1. Image preprocessing: {(time.time() - img_prep_start)*1000:.2f}ms")
        
        # 2. Feature extraction
        feat_extract_start = time.time()
        img_feats = self.extract_feat(imgs)
        print(f"[DEBUG] 2. Image feature extraction: {(time.time() - feat_extract_start)*1000:.2f}ms")
        ...
        if self.use_lidar:
            ...
            print(f"[DEBUG] 3a. LiDAR processing: {(time.time() - lidar_start)*1000:.2f}ms")
        if self.use_radar:
            radar_start = time.time()
            radar_xyz_feat = self.svfe_radar(batch_inputs['radar_voxel_feats'], batch_inputs['radar_voxel_coords'])
            print(f"[DEBUG] 3b. Radar processing: {(time.time() - radar_start)*1000:.2f}ms")
        print(f"[DEBUG] 3. Total sensor processing: {(time.time() - sensor_start)*1000:.2f}ms")
        
        # 4. View transformation
        view_transform_start = time.time()
        ...
        print(f"[DEBUG] 4. View transformation: {(time.time() - view_transform_start)*1000:.2f}ms")
        ...
        print(f"[DEBUG] 5. Gaussian loss: {(time.time() - gaussian_start)*1000:.2f}ms")
        ...
        if self.use_geometry and type(self.use_geometry) != bool and xyz_volumes[0].requires_grad:
            ...
            print(f"[DEBUG] 6. Geometry loss: {(time.time() - geometry_start)*1000:.2f}ms")
            
            # 7. Occupancy head
            occ_head_start = time.time()
            occ_result = self.occ_head(xyz_volumes)
            print(f"[DEBUG] 7. Occupancy head: {(time.time() - occ_head_start)*1000:.2f}ms")
    ...
    def loss(self, batch_inputs, batch_data_samples):
        ...
        print(f"[LOSS] Multiscale supervision: {(time.time() - supervision_start)*1000:.2f}ms")
        ...
        print(f"[LOSS] Data preparation for loss: {(time.time() - data_prep_start)*1000:.2f}ms")
        ...
        print(f"[LOSS] Uncertainty computation: {(time.time() - uncertainty_start)*1000:.2f}ms")
        ...
        print(f"[LOSS] Multi-level loss computation: {(time.time() - loss_computation_start)*1000:.2f}ms")
        
        # Add additional losses
        final_loss_start = time.time()
        if type(geometric_loss) != bool:
            loss['geometric_loss'] = geometric_loss
        loss['gaussian_loss'] = gaussian_loss
        print(f"[LOSS] Final loss assembly: {(time.time() - final_loss_start)*1000:.2f}ms")
