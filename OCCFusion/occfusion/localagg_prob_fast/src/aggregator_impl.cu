    ...
    const dim3 grid)
{
	...
	// Gaussian이 영향을 주는 3D Voxel 영역을 계산하는 함수
    getRect(points_xyz + 3 * idx, radii + 3 * idx, rect_min, rect_max, grid);
    // 영향 영역의 최소, 최대 좌표가 출력됨 (rect_min, rect_max)

	// Key : Gaussian 하나가 Cover하는 Grid의 (Flattened) Location
	// Value : Gaussian의 Index
	...
	bool debug)
{
	...
	if (num_rendered > 0)
		identifyTileRanges << <(num_rendered + 255) / 256, 256 >> > (
			num_rendered, // 전체 gaussian-voxel 쌍 개수
			binningState.point_list_keys, // sorted voxel location lists
			imgState.ranges); // gaussian range in each voxel
	CHECK_CUDA(, debug)
