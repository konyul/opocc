	...
	float* __restrict__ out_probability)
{
	...
	// gaussian index range from x ~ y in certain voxel
	for (int i = range.x; i < range.y; i++)
	{
		...
		float prob = powf(2 * 3.1415926535, -1.5) * powf(deter, 0.5) * power * opas[gs_idx]; // gs_idx에 따라서 scaling을 해주어 pdf를 만듦

		for (int ch = 0; ch < CHANNELS; ch++)
		{
			C[ch] += semantic[CHANNELS * gs_idx + ch] * prob;
		}
		bin_logit = (1 - power) * bin_logit; // empty voxel prob accumulation
		density = power + density; // 
		prob_sum = prob + prob_sum; // splatted gaussian summation
	}
	...
	if (prob_sum > 1e-9) {
		for (int ch = 0; ch < CHANNELS; ch++)
			out_logits[idx * CHANNELS + ch] = C[ch] / prob_sum; // calculate expectation of c
	} else {
		for (int ch = 0; ch < CHANNELS - 1; ch++)
			out_logits[idx * CHANNELS + ch] = 1.0 / (CHANNELS - 1); // fallback : uniform : 해당 voxel에 기여하는 Gaussian이 사실상 없는 경우
	}
	// idx : 현재 thread가 담당하는 voxel 번호
...
{
	renderCUDA<NUM_CHANNELS> << <(N + 255) / 256, 256 >> > ( // GPU Execution with N+255 / 256 Grid / 256 Blocks
		N, 
		pts,
		points_int,
		grid,
